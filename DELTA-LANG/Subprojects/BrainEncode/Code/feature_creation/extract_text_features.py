...
#This function:
# - tokenizes the story
# - scores the event marker at each tokem
# - fills missing positions
# - resamples the result to the fMRI grid

#The core idea is to use an LLM natural functionality "assigning a log prob to every token in its vocab." to extract what the probability of the next token being a event boundary is.
def boundary_lp(ds, tok, model, lookahead: int, device="cuda"):
    """
    lp_word[i]  =  log_p(EVENT_MARKER | context that includes *lookahead*
                 future tokens beyond word i)
    """


    #tokenize the words in ds.data
    enc = tok(ds.data.tolist(),
                       is_split_into_words=True,
                       add_special_tokens=False,
                       return_tensors="pt")
    #the whole story as a list of token indices (batch sizte is one)
    seq = enc.input_ids[0].tolist()

    # Build a mapping from each token position back to its original word index.
    # This is how token-level scores are later collapsed back to words.
    word_for_tok = enc.word_ids()

    #Token level output array initialized with nan
    lp_tok  = np.full(len(seq),     np.nan, np.float32)

    #Word level outtput array initialized with nan
    lp_word = np.full(len(ds.data), np.nan, np.float32)

    # build the chat prefix once
    #System prompt give the instructions about event boundaries
    #PROMPT CORE is the user side prefix BEFORE the story content
    msgs = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {"role": "user",   "content": PROMPT_CORE},
    ]

    #Convert the chat prompt into token IDs using the tokenizers chat template.
    # `add_generation_prompt=True` Tells the model "start here".
    chat_ids = tok.apply_chat_template(
        msgs, add_generation_prompt=True,
        return_tensors="pt"
    ).tolist()[0]

    #set the context limit, get it if avialable otherwise fall back to a default value
    ctx_lim = getattr(model.config, "max_position_embeddings",
                      CTX_LIMIT_DEFAULT)
    # Compute how many story tokens can fit after the chat prefix.
    # -2 is just a small safety buffer.
    room = ctx_lim - len(chat_ids) - 2

    #We experimented with lookahead >0 but found lookahead = 0 gives the best
    # lookahead 0 means: score the event marker using only the story up to the current point
    #>0 means: let the score depend on some future tokens too
    if room <= lookahead:
        raise ValueError("look‑ahead larger than available context room")

    # ---- sliding window
    # process the story in chuncks
    for s in range(0, len(seq), room - lookahead):
        #make sure we dont go beyond the story limit
        e_main = min(s + room - lookahead, len(seq) - lookahead)

        #currenr block
        block  = seq[s : e_main + lookahead]           # +future

        # Concatenate prompt prefix + story block and move it to GPU/CPU.
        ids    = torch.tensor([chat_ids + block], device=device)

        # run one forward pass without gradeints
        with torch.no_grad():
            logits = model(ids,use_cache=False).logits[0]

        # logits are the models raw score for every possible next token so:
        # logits[position, token_id] means "how much does the model like token_id as the next token at this position
        # to get the log_prob pass it through log softmax
        #why log? - numerical stability, LLMs work in log space
        lps = torch.log_softmax(logits, -1).cpu().numpy()

        #offset cause by the chat instructions
        off = len(chat_ids)
        ...
        for j in range(0, len(block) - lookahead):  # j = position of w_t
            tgt_tok_idx = s + j  # absolute index in the full story
            #
            ...
            # read the log prob of our special marker beiong the next token at every position
            score = lps[src_row, marker_id]

            #store it at the token level
            lp_tok[tgt_tok_idx] = score

            #store it at the word level
    ...
    #Sometimes there are a small amount of missing tokens, fill them with linear interpolation
    #NOTE: ONLY DO THIS IF YOU KNOW WHAT YOU ARE DOING, THERE SHOULD NOT BE A LOT OF MISSING vALUES
    if mask.any():
    ...
    #the actual downsampliung happens here
    # using a windowed sinc interpolation filter
    # we use a window of 1 (small) because event boundaries should be sharp
    lp_tr = np.asarray(
