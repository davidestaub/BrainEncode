...
zs = lambda v: (v-v.mean(0))/v.std(0) ## z-score function == standardise
...
def ridge(stim, resp, alpha, singcutoff=1e-10, normalpha=False, logger=ridge_logger):
    ...
    print("Response has shape",resp.shape,np.nan_to_num(resp).shape)
...
# The main methods use to find the weights, they perform regularized ridge regression
# with k-fold cross validation to estimate the best alpha values (a HYPERPARAMETER i.e not trainable)
def bootstrap_ridge(Rstim, Rresp, Pstim, Presp, alphas, nboots, chunklen, nchunks,
    ...
    print("nresp, nvox = ", nresp, nvox)
    print("test test :", Rstim.shape)
    # nresp, nvox =  8673 95556
    ...
    for bi in counter(range(nboots), countevery=1, total=nboots):
        ...
        # Dividing the indices of the data (allinds) into chunks of size 'chunklen'
        indchunks = list(zip(*[iter(allinds)] * chunklen))

        # Shuffling these chunks randomly
        random.shuffle(indchunks)

        # Selecting the first 'nchunks' shuffled chunks as the held-out test set (heldinds)
        heldinds = list(itools.chain(*indchunks[:nchunks]))

        # The remaining indices form the training set (notheldinds)
        notheldinds = list(set(allinds) - set(heldinds))

        # Record the held-out indices for this bootstrap iteration
        valinds.append(heldinds)

        # In this bootstrapping loop, a held-out test set is selected for each iteration.
        # The process involves:
        # 1. Dividing the indices of the data (allinds) into chunks of size 'chunklen'.
        # 2. Shuffling these chunks randomly.
        # 3. Selecting the first 'nchunks' shuffled chunks as the held-out test set (heldinds).
        # 4. The remaining indices form the training set (notheldinds).
        # 5. Flattening the train and test chunks again
    ...
    if not single_alpha:
        ...
        if joined is None:
            # Average correlations across bootstraps to find the best alpha for each voxel
            meanbootcorrs = allRcorrs.mean(2)

            # Find the index of the alpha that gives the maximum average correlation for each voxel
            bestalphainds = np.argmax(meanbootcorrs, 0)

            # Use fancy indexing to select the best alpha for each voxel
        ...
        if nboots == 0:
            ...
            # Mean across all voxels and bootstraps to find the best single alpha
            meanbootcorr = allRcorrs.mean(2).mean(1)
            bestalphaind = np.argmax(meanbootcorr)
            bestalpha = alphas[bestalphaind]

        # Apply the best alpha to all voxels (single alpha scenario)
